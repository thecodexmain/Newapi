...
def login_instagram():
    ...
    try:
        ...
        if data.get('cookies'):
            # Cookie-based login - handle both array and string format
            ...
            return jsonify({'error': 'Either cookies, cookie_string, or username/password required'}), 400
...
def list_sessions():
    """List all active sessions (admin only - add auth in production)"""
    with session_lock:
        return jsonify({
